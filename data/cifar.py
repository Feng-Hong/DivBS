...
def CIFAR100_half(config, logger):
    ...
    # dst_train = torch.utils.data.Subset(dst_train, indices[:split])
    dst_train.data = dst_train.data[indices[:split]]
    dst_train.targets = [dst_train.targets[i] for i in indices[:split]]
